    ...
    <div className="max-w-5xl space-y-5 animate-fade-in">
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-ink-100">
            {blogs.map((blog) => (
              <div key={blog._id} className="flex items-center gap-4 p-4 hover:bg-ink-50 transition-colors">
                {/* Thumbnail */}
      ...
      {pages > 1 && (
        <div className="flex justify-center gap-2">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="btn-ghost disabled:opacity-40">← Prev</button>
          <span className="font-ui text-sm text-ink-500 self-center">Page {page} of {pages}</span>
          <button onClick={() => setPage((p) => Math.min(pages, p + 1))} disabled={page === pages} className="btn-ghost disabled:opacity-40">Next →</button>
        </div>
      )}
    </div>
  );
