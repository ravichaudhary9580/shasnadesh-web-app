...
export default function BlogDetail() {
  ...
  if (notFound || !blog) return (
    <div className="min-h-screen bg-ink-50 flex flex-col justify-between">
      <SEO 
        title="404 - पोस्ट नहीं मिली | Shasnadesh Updates"
  ...
  return (
    <div className="min-h-screen bg-ink-50">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10 relative overflow-hidden">
        {/* Whole-Blog Watermark Layer */}
        <BlogWatermarkOverlay watermark={blog.watermark} />

        <div className="relative z-10 space-y-8">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-2 text-sm font-ui text-ink-400">
          ...
          <div>
            ...
            {/* Title */}
            <h1 className={`font-display font-bold text-ink-900 leading-tight mb-4 text-balance ${isHindi ? "font-hindi text-3xl md:text-4xl" : "text-3xl md:text-5xl"
              }`}>
              {blog.title}
            </h1>

            {blog.excerpt && (
              <p className={`text-xl text-ink-500 font-body leading-relaxed ${isHindi ? "font-hindi" : ""}`}>
                {blog.excerpt}
              </p>
            )}
          </div>
          ...
          {/* Table of Contents */}
          {headings.length > 0 && (
            <div className="p-5 sm:p-6 bg-white/90 backdrop-blur-xs border border-ink-100 rounded-2xl shadow-sm animate-fade-in">
